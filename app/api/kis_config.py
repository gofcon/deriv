...
class KISConfig:
    ...
    def __init__(self, config_dir: str = None):
        ...
        if not os.path.exists(config_file):
            # 파일이 없어도 에러를 내지 않고 나중에 체크하거나 빈 딕셔너리로 초기화
            # 하지만 kis_config.py의 로직상 파일이 있어야 함.
            # 예외 처리를 추가하거나 사용자가 파일을 생성했다고 가정.
            pass
        ...
        try:
            with open(config_file, encoding="UTF-8") as f:
             ...
             raise FileNotFoundError(f"Config file not found at {config_file}. Please ensure ~/KIS/config/kis_devlp.yaml exists.")
